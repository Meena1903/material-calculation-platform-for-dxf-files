...
class TakeoffExporter:
    """Generates structured JSON, CSV BOQ, and submission ZIP package with error handling."""
    ...
    def __init__(self, output_dir: str = "outputs"):
        ...
        try:
            ...
            exporter_logger.warning(f"[EXPORT INIT WARNING] Could not create output dir '{self.output_dir}': {e}")
        exporter_logger.info(f"[EXPORT INIT] Initialized TakeoffExporter with output_dir='{self.output_dir}'")

    def export_to_json(self, result: TakeoffResult, file_path: Optional[str] = None) -> str:
        """Serialize TakeoffResult to structured output_takeoff.json with error handling."""
        if not file_path:
        ...
        try:
            ...
            exporter_logger.error(f"[EXPORT JSON PERMISSION ERROR] Cannot write to '{file_path}' (file locked or read-only): {pe}")
            return file_path
        except Exception as e:
            exporter_logger.error(f"[EXPORT JSON EXCEPTION] Failed exporting JSON to '{file_path}': {e}")
            return file_path

    def export_to_csv(self, result: TakeoffResult, file_path: Optional[str] = None) -> str:
        """Export BOQ line items to standard output_boq.csv with error handling."""
        if not file_path:
        ...
        try:
            ...
            exporter_logger.error(f"[EXPORT CSV PERMISSION ERROR] Cannot write to '{file_path}' (file may be open in Excel): {pe}")
            return file_path
        except Exception as e:
            exporter_logger.error(f"[EXPORT CSV EXCEPTION] Failed exporting CSV to '{file_path}': {e}")
            return file_path
    ...
    def create_submission_zip(
        ...
        """Package source code, schemas, output artifacts, requirements, and README into a submission zip with error handling."""
        ...
        try:
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                for root, dirs, files in os.walk(base_dir):
                    ...
                    for file in files:
                        if file.endswith((".pyc", ".pyo", ".pyd", ".zip")):
                            continue
                        ...
                        try:
                            ...
                            exporter_logger.debug(f"[ZIP FILE SKIP] Could not add '{full_path}' to zip: {file_err}")

            size_mb = round(os.path.getsize(zip_path) / (1024 * 1024), 2)
            exporter_logger.info(f"[EXPORT STEP 3: ZIP SUCCESS] Packaged {file_count} files ({size_mb} MB) into '{zip_path}'")
            return zip_path
        except Exception as e:
            exporter_logger.error(f"[EXPORT ZIP EXCEPTION] Failed creating submission zip '{zip_path}': {e}")
            return zip_path
